...
const findOrCreateCategory = async (categoryId, categoryName) => {
    if (categoryId) {
...
const parseBooleanValue = (value) => value === true || value === "true";
...
export const createSellerProduct = async (req, res) => {
    try {
        ...
        if (!userId)
            return;
        const seller = await getSellerProfile(userId);
        const { name, description, price, stock, categoryId, categoryName, isAvailable } = req.body;
...
export const getSellerReceipts = async (req, res) => {
    try {
        const userId = ensureUserId(req, res);
        if (!userId)
            return;
...
export const getSellerChatWithUser = async (req, res) => {
    try {
